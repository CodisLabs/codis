start_server {tags {"zset"}} {
    ...
    proc basics {encoding} {
        ...
        test "ZUNIONSTORE against non-existing key doesn't set destination - $encoding" {
            r del zsetxdl
            assert_equal 0 [r zunionstore zsetyfX 1 zsetxdl]
            assert_equal 0 [r exists zsetyfX]
        }

        test "ZUNIONSTORE with empty set - $encoding" {
            r del zsetxdl zsetyfX
            r zadd zsetxdl 1 a
            r zadd zsetxdl 2 b
            r zunionstore zsetzva 2 zsetxdl zsetyfX
            r zrange zsetzva 0 -1 withscores
        } {a 1 b 2}

        test "ZUNIONSTORE basics - $encoding" {
            r del zsetxdl zsetyfX zsetzva
            r zadd zsetxdl 1 a
            r zadd zsetxdl 2 b
            r zadd zsetxdl 3 c
            r zadd zsetyfX 1 b
            ...
            assert_equal 4 [r zunionstore zsetzva 2 zsetxdl zsetyfX]
            assert_equal {a 1 b 3 d 3 c 5} [r zrange zsetzva 0 -1 withscores]
        }

        test "ZUNIONSTORE with weights - $encoding" {
            assert_equal 4 [r zunionstore zsetzva 2 zsetxdl zsetyfX weights 2 3]
            assert_equal {a 2 b 7 d 9 c 12} [r zrange zsetzva 0 -1 withscores]
        }
        ...
        test "ZUNIONSTORE with a regular set and weights - $encoding" {
            ...
            assert_equal 4 [r zunionstore zsetzva 2 zsetxsm zsetyfX weights 2 3]
            assert_equal {a 2 b 5 c 8 d 9} [r zrange zsetzva 0 -1 withscores]
        }

        test "ZUNIONSTORE with AGGREGATE MIN - $encoding" {
            assert_equal 4 [r zunionstore zsetzva 2 zsetxdl zsetyfX aggregate min]
            assert_equal {a 1 b 1 c 2 d 3} [r zrange zsetzva 0 -1 withscores]
        }

        test "ZUNIONSTORE with AGGREGATE MAX - $encoding" {
            assert_equal 4 [r zunionstore zsetzva 2 zsetxdl zsetyfX aggregate max]
            assert_equal {a 1 b 2 c 3 d 3} [r zrange zsetzva 0 -1 withscores]
        }

        test "ZINTERSTORE basics - $encoding" {
            assert_equal 2 [r zinterstore zsetzva 2 zsetxdl zsetyfX]
            assert_equal {b 3 c 5} [r zrange zsetzva 0 -1 withscores]
        }

        test "ZINTERSTORE with weights - $encoding" {
            assert_equal 2 [r zinterstore zsetzva 2 zsetxdl zsetyfX weights 2 3]
            assert_equal {b 7 c 12} [r zrange zsetzva 0 -1 withscores]
        }
        ...
        test "ZINTERSTORE with a regular set and weights - $encoding" {
            ...
            r sadd zsetxsm b
            r sadd zsetxsm c
            assert_equal 2 [r zinterstore zsetzva 2 zsetxsm zsetyfX weights 2 3]
            assert_equal {b 5 c 8} [r zrange zsetzva 0 -1 withscores]
        }

        test "ZINTERSTORE with AGGREGATE MIN - $encoding" {
            assert_equal 2 [r zinterstore zsetzva 2 zsetxdl zsetyfX aggregate min]
            assert_equal {b 1 c 2} [r zrange zsetzva 0 -1 withscores]
        }

        test "ZINTERSTORE with AGGREGATE MAX - $encoding" {
            assert_equal 2 [r zinterstore zsetzva 2 zsetxdl zsetyfX aggregate max]
            assert_equal {b 2 c 3} [r zrange zsetzva 0 -1 withscores]
        }
    ...
    test {ZINTERSTORE regression with two sets, intset+hashtable} {
        ...
        r sadd zsetxdl a
        r sadd zsetyfX 10
        r zinterstore zsetzva 2 setxdl setyfX
    } {0}

    test {ZUNIONSTORE regression, should not create NaN in scores} {
        r del zsetxdl zsetyfX
        r zadd zsetxdl -inf neginf
        r zunionstore zsetyfX 1 zsetxdl weights 0
        r zrange zsetyfX 0 -1 withscores
    } {neginf 0}

    test {ZINTERSTORE #516 regression, mixed sets and ziplist zsets} {
        r del zsetxdl zsetyfX zsetzva zsetxsm
        r sadd zsetxdl 100 101 102 103
        r sadd zsetyfX 100 200 201 202
        r zadd zsetzva 1 500 1 501 1 502 1 503 1 100
        r zinterstore zsetxsm 3 zsetxdl zsetyfX zsetzva WEIGHTS 0 0 1
        r zrange zsetxsm 0 -1
    } {100}
