...
### 如何停止迁移任务
Codis迁移数据时是不会影响服务的，因此强烈建议不要手动终止迁移。一旦slot的状态已经进入migrate，迁移是不能中断的，因为已经有一部分数据和新写入的增量数据在新的group了。并且两边都有数据的话继续迁移和回滚是等价的，所以回滚也是没有意义的。如果因为其他原因，导致slot卡在pre_migrate（需要确保该slot从未进入过migrate状态否则中止任务会丢数据），或依然是online状态但迁移不断的报错，可以在zk中删除/zk/codis/db_%s/migrate_tasks下的子节点。其他的节点不要动。
