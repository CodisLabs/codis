...
static lazyReleaseIterator *createLazyReleaseIterator(robj *val) {
...
static void freeLazyReleaseIterator(lazyReleaseIterator *it) {
    if (it->val != NULL) {
        decrRefCount(it->val);
    }
    zfree(it);
...
static int lazyReleaseIteratorHasNext(lazyReleaseIterator *it) {
    return it->val != NULL;
}

static void lazyReleaseIteratorScanCallback(void *data, const dictEntry *de) {
    void **pd = (void **)data;
    list *l = pd[0];
...
static void lazyReleaseIteratorNext(lazyReleaseIterator *it, int step) {
    robj *val = it->val;
    ...
    if (val->type == OBJ_HASH || val->type == OBJ_SET) {
        ...
        if (dictSize(ht) <= limit) {
            ...
            void *pd[] = {ll};
            do {
                it->cursor = dictScan(ht, it->cursor, lazyReleaseIteratorScanCallback, NULL, pd);
            } while (it->cursor != 0 &&
                    listLength(ll) < (unsigned long)step &&
                    (--loop) >= 0);

            while (listLength(ll) != 0) {
                listNode *head = listFirst(ll);
                robj *field = listNodeValue(head);
                dictDelete(ht, field->ptr);
                decrRefCount(field);
                listDelNode(ll, head);
            }
            listRelease(ll);
...
static int lazyReleaseIteratorRemains(lazyReleaseIterator *it) {
    robj *val = it->val;
    if (val == NULL) {
        return 0;
    }
...
static int slotsmgrtLazyReleaseStep(int step) {
...
static void slotsmgrtLazyReleaseMicroseconds(long long usecs) {
...
void slotsmgrtLazyReleaseCleanup() {
    long long ops = server.stat_numcommands - lazy_release_opt.last_numcommands;
    if (ops < 0) {
        ops = 0;
    }
    if (ops > 30) {
        lazy_release_opt.step = 1 + (1000 / ops) * 3;
    } else {
        lazy_release_opt.step = 100;
    }
    lazy_release_opt.last_numcommands = server.stat_numcommands;

    long long usecs = lazy_release_opt.step / 10 * 100;
    if (usecs != 0) {
...
void slotsmgrtLazyReleaseIncrementally() {
    slotsmgrtLazyReleaseStep(lazy_release_opt.step);
}
...
static void singleObjectIteratorScanCallback(void *data, const dictEntry *de) {
    ...
    sds eles[2] = {NULL, NULL};
    switch (o->type) {
    case OBJ_HASH:
        eles[0] = dictGetKey(de);
        eles[1] = dictGetVal(de);
        break;
    case OBJ_SET:
        eles[0] = dictGetKey(de);
        break;
    ...
    for (int i = 0; i < 2; i++) {
        if (eles[i] != NULL) {
            robj *obj = createStringObject(eles[i], sdslen(eles[i]));
...
static int getUint64FromRawStringObject(robj *o, uint64_t *p) {
...
static long numberOfRestoreCommandsFromObject(robj *val, long long maxbulks) {
...
static long estimateNumberOfRestoreCommands(redisDb *db, robj *key, long long maxbulks) {
...
static int singleObjectIteratorNext(client *c, singleObjectIterator *it,
    ...
    if (it->stage == STAGE_CHUNKED) {
        ...
        if (val->type == OBJ_ZSET) {
            ...
            do {
                if (node != NULL) {
                    robj *field = createStringObject(node->ele, sdslen(node->ele));
        ...
        while (listLength(ll) != 0) {
            ...
            addReplyBulk(c, obj);
            /* The ll->free has been setted to decrRefCountVoid(). */
            listDelNode(ll, head);
        }
        listRelease(ll);
...
static batchedObjectIterator *createBatchedObjectIterator(dict *hash_slot,
        struct zskiplist *hash_tags, long long timeout, unsigned int maxbulks,
        unsigned int maxbytes) {
...
static int batchedObjectIteratorAddKey(redisDb *db, batchedObjectIterator *it, robj *key) {
    ...
    while (node != NULL && node->score == (double)crc) {
        robj *more = createStringObject(node->ele, sdslen(node->ele));
        node = node->level[0].forward;
        if (dictAdd(it->keys, more, NULL) != C_OK) {
            decrRefCount(more);
            continue;
        }
        listAddNodeTail(it->list, createSingleObjectIterator(more));
        it->estimate_msgs += estimateNumberOfRestoreCommands(db, more, it->maxbulks);
    }
...
static int createSlotsmgrtAsyncClient(int db, char *host, int port, long timeout) {
...
static slotsmgrtAsyncClient *getOrCreateSlotsmgrtAsyncClient(int db, char *host, int port, long timeout) {
    slotsmgrtAsyncClient *ac = getSlotsmgrtAsyncClient(db);
    if (ac->c != NULL) {
...
static long slotsmgrtAsyncNextMessagesMicroseconds(slotsmgrtAsyncClient *ac, long atleast, long long usecs) {
    ...
    long msgs = 0;
    while (batchedObjectIteratorHasNext(it) &&
            getClientOutputBufferMemoryUsage(ac->c) < it->maxbytes) {
...
void slotsmgrtExecWrapperCommand(client *c) {
    ...
    if ((cmd->arity > 0 && cmd->arity != c->argc - 2) ||
            (c->argc - 2 < -cmd->arity)) {
        addReplyLongLong(c, -1);
        addReplyErrorFormat(c, ERRFMT_WRONG_ARGC, (char *)c->argv[2]->ptr);
        return;
    }
...
static int slotsrestoreAsyncHandle(client *c) {
    ...
    if (!strcasecmp(cmd, "object")) {
        ...
        robj *val = rdbLoadObject(type, &payload, key);
        if (val == NULL) {
    ...
    if (!strcasecmp(cmd, "zset")) {
        ...
        for (int i = 0, j = 0; i < xargc; i += 2, j++) {
            dictEntry *de = dictUnlink(zset->dict, xargv[i]->ptr);
            if (de != NULL) {
                /* Get the score in order to delete from the skiplist later. */
                double score = *(double *)dictGetVal(de);
                /* Delete from the hash table and later from the skiplist.
                 *
                 * Note that the order is important: deleting from the zsl
                 * actually releases the SDS string representing the element,
                 * which is shared between the skiplist and the hash table, so
                 * we need to delete from the skiplist as the final step.*/
                dictFreeUnlinkedEntry(zset->dict, de);
                serverAssert(zslDelete(zset->zsl, score, xargv[i]->ptr, NULL));
            }
            sds ele = sdsdup(xargv[i]->ptr);
            zskiplistNode *znode = zslInsert(zset->zsl, scores[j], ele);
            serverAssert(dictAdd(zset->dict, ele, &(znode->score)) == DICT_OK);
        }
        zfree(scores);
